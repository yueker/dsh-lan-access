...
const LAN_PRIVILEGED_CLIENT = `<script>(function () {
  try {
    var CONNECTION_ID = "@deepseek-ai/dsh-client-connection";
    var patchRegistration = function (registration) {
      if (!registration || registration.id !== CONNECTION_ID || typeof registration.factory !== "function") return registration;
      var originalFactory = registration.factory;
        ...
        factory: function (require) {
          ...
          if (exports && typeof exports.apply === "function") {
            ...
            exports.apply = function (ctx, config) {
              // The providing plugin cannot read its own service back through
              // ctx.get() in this cordis build, so adjust the handle at
              // provide() time instead.
              var adjust = function (value) {
                if (value && typeof value === "object" && "isLoopback" in value && value.isLoopback !== true) {
                  value.isLoopback = true;
                }
              };
              var provideWrapped = false;
              try {
                var originalProvide = ctx.provide;
                if (typeof originalProvide === "function" && !originalProvide.__dshLanPrivilegedWrapped) {
                  var wrappedProvide = function (name, value) {
                    if (name === "connection") adjust(value);
                    return originalProvide.call(this, name, value);
                  };
                  wrappedProvide.__dshLanPrivilegedWrapped = true;
                  ctx.provide = wrappedProvide;
                  provideWrapped = ctx.provide === wrappedProvide;
                } else if (originalProvide && originalProvide.__dshLanPrivilegedWrapped) {
                  provideWrapped = true;
                }
              } catch (e) {
                console.warn("[dsh-lan-access] provide wrap unavailable:", e);
              }
              var result = originalApply(ctx, config);
              if (!provideWrapped) {
                // Fallback: resolve the service shortly after apply completes.
                Promise.resolve().then(function () {
                  try {
                    var connection = ctx.get("connection");
                    if (connection) adjust(connection);
                    else console.warn("[dsh-lan-access] fallback: connection service still unresolved");
                  } catch (e) {
                    console.warn("[dsh-lan-access] fallback adjust failed:", e);
                  }
                });
              }
              return result;
            ...
            console.warn("[dsh-lan-access] connection exports have no apply()", exports && Object.keys(exports));
          }
          return exports;
        }
      });
    };
    var wrapLoad = function (loader) {
      if (!loader || typeof loader.load !== "function" || loader.load.__dshLanPrivilegedWrapped) return;
      var originalLoad = loader.load;
      var wrappedLoad = function (registration) {
        return originalLoad.call(this, patchRegistration(registration));
      };
      wrappedLoad.__dshLanPrivilegedWrapped = true;
      loader.load = wrappedLoad;
    };
    var wrap = function (loader) {
      if (!loader) return loader;
      wrapLoad(loader);
      if (typeof loader.create === "function" && !loader.create.__dshLanPrivilegedWrapped) {
        var originalCreate = loader.create;
        var wrappedCreate = function () {
          var result = originalCreate.apply(this, arguments);
          // ClientModuleSystem's constructor replaces target.load with the live
          // registration sink. Re-wrap that replacement before create returns.
          wrapLoad(loader);
          return result;
        };
        wrappedCreate.__dshLanPrivilegedWrapped = true;
        loader.create = wrappedCreate;
      }
      return loader;
    };
    var value = wrap(window.__ModuleLoader__);
    Object.defineProperty(window, "__ModuleLoader__", {
      configurable: true,
      enumerable: true,
      get: function () { return value; },
      set: function (next) { value = wrap(next); }
    });
  } catch (e) {
    /* never break page boot */
  }
...
export function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.tapIndex((html) => {
    ...
    if (config?.allowPrivilegedFromLan === true) out += LAN_PRIVILEGED_CLIENT;
